...
string getString(const LPXLOPER12& oper, const size_t i = 0,
                 const size_t j = 0) {
  if (!oper) return "";
...
void resize(LPXLOPER12& oper, const size_t rows, const size_t cols) {
  ...
  oper->val.array.lparray =
      (LPXLOPER12)GetTempMemory(rows * cols * sizeof(XLOPER12));
  LPXLOPER12 nilOper = TempStr12("");
  fill(oper->val.array.lparray, oper->val.array.lparray + rows * cols,
       *nilOper);
}

//  Set string in position (i, j)
void setString(LPXLOPER12& oper, const string& str, const size_t i = 0,
               const size_t j = 0) {
...
//  Number in position (i, j) or infinity if invalid
void setNum(LPXLOPER12& oper, const double num, const size_t i = 0,
            const size_t j = 0) {
  XLOPER12& nOper = oper->val.array.lparray[i * getCols(oper) + j];
...
vector<string> to_strVector(const LPXLOPER12& oper) {
  ...
  if (oper->xltype == xltypeStr) {
    ...
    for (size_t i = 0; i < getRows(oper); ++i)
      for (size_t j = 0; j < getCols(oper); ++j) {
...
LPXLOPER12 from_strVector(const vector<string>& strv,
  ...
  oper->val.array.lparray =
      (LPXLOPER12)GetTempMemory(strv.size() * sizeof(XLOPER12));
  transform(strv.begin(), strv.end(), oper->val.array.lparray,
            [](const string& s) { return *TempStr12(s); });

  return oper;
}

LPXLOPER12 from_labelsAndNumbers(const vector<string>& labels,
...
LPXLOPER12 from_matrix(const matrix<double>& mat) {
  ...
  resize(oper, n, m);
  for (size_t i = 0; i < n; ++i)
    for (size_t j = 0; j < m; ++j) setNum(oper, mat[i][j], i, j);

  return oper;
}

LPXLOPER12 from_labelledMatrix(const vector<string>& rowLabels,
                               const vector<string>& colLabels,
                               const matrix<double>& mat) {
  const size_t n = rowLabels.size(), m = colLabels.size();
  if (n == 0 || m == 0 || n != mat.rows() || m != mat.cols())
  ...
  for (size_t i = 0; i < n; ++i)
    for (size_t j = 0; j < m; ++j) setNum(oper, mat[i][j], i + 1, j + 1);

  return oper;
}

LPXLOPER12 from_labelledMatrix(const vector<double>& rowLabels,
                               const vector<double>& colLabels,
                               const matrix<double>& mat) {
  const size_t n = rowLabels.size(), m = colLabels.size();
  if (n == 0 || m == 0 || n != mat.rows() || m != mat.cols())
    return TempErr12(xlerrNA);
...
LPXLOPER12 from_labelledMatrix(const vector<string>& rowLabels,
                               const vector<string>& colLabels,
                               const matrix<double>& mat,
                               const string& firstLineLabel,
                               const vector<double>& firstLine) {
  const size_t n = rowLabels.size(), m = colLabels.size();
  if (n == 0 || m == 0 || n != mat.rows() || m != mat.cols() ||
      m != firstLine.size())
    return TempErr12(xlerrNA);
  ...
  for (size_t i = 0; i < n; ++i)
    for (size_t j = 0; j < m; ++j) setNum(oper, mat[i][j], i + 2, j + 1);
