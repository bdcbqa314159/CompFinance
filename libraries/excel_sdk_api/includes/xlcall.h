...
** Return codes
**
** These values can be returned from Excel4(), Excel4v(), Excel12() or
*Excel12v().
*/
...
#define xlretInvAsynchronousContext \
  256                           /* invalid asynchronous function handle */
...
#define xleventCalculationCanceled           \
  2 /* Fires when calculation is interrupted \
     */
...
int pascal Excel12v(int xlfn, LPXLOPER12 operRes, int count,
                    LPXLOPER12 opers[]);
...
typedef int(CALLBACK* PXL_HPC_ASYNC_CALLBACK)(LPXLOPER12 operAsyncHandle,
                                              LPXLOPER12 operReturn);
...
#define xlGetInst                                                         \
  (7 | xlSpecial) /* Returns application's hinstance as an integer value, \
                     supported on 32-bit platform only */
...
#define xlAsyncReturn \
  (16 | xlSpecial) /*Set return value from an asynchronous function call*/
#define xlEventRegister (17 | xlSpecial) /*Register an XLL event*/
#define xlRunningOnCluster \
  (18 | xlSpecial) /*Returns true if running on Compute Cluster*/
#define xlGetInstPtr                                                         \
  (19 | xlSpecial) /* Returns application's hinstance as a handle, supported \
